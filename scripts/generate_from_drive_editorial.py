...
The Drive document is treated as a private editorial seed, not a factual source. Current
public web information is researched during generation and private Drive identifiers are
never written into public article assets.
"""
...
def mark_started(state: dict[str, Any], doc: dict[str, Any]) -> None:
    ...
    record.update({"modifiedTime": doc.get("modifiedTime"), "status": "processing", "startedAt": base.now_iso(), "retry_count": int(record.get("retry_count", 0))})
...
def mark_finished(state: dict[str, Any], doc: dict[str, Any], status: str, detail: dict[str, Any], permanent: bool) -> None:
    ...
    record.update({"modifiedTime": doc.get("modifiedTime"), "status": status, "finishedAt": base.now_iso(), "retry_count": retry, **safe_detail})
    state["updatedAt"] = base.now_iso()
...
def call_openai_once(doc: dict[str, Any], source_text: str, source_processing: dict[str, Any], timer: Any, mock_timeout: bool) -> dict[str, Any]:
    ...
    user_input = {"today": date.today().isoformat(), "seed_title": doc.get("name"), "seed_text": source_text, "source_processing": source_processing, "allowed_links": base.ALLOWED_LINKS, "existing_article_titles": _existing_titles(), "editorial_goal": "Use the private draft only as a seed. Research current public news/trends and rebuild the article for HDN's current audience."}
...
def _published_payload(data: dict[str, Any]) -> str:
    """Return only text that can reach public JP/EN article or social assets."""
...
def validate_sanitized_output(data: dict[str, Any]) -> list[str]:
    """Fail only when sensitive material remains in the generated public payload.

    AI confidentiality_flags are diagnostic notes about what existed in the private seed;
    they are not a publication veto. The public output itself must pass deterministic
    sensitive-pattern checks before files are written.
    """
    return base.confidentiality_flags("", _published_payload(data))
...
def build_article(data: dict[str, Any], doc: dict[str, Any]) -> str:
    ...
    lines = ["---", f"title: {base.yaml_string(str(data['title']).strip())}", f"description: {base.yaml_string(description)}", f"publishedAt: {today}", f"updatedAt: {today}", f"category: {base.yaml_string(str(data.get('category') or 'クリニック経営'))}", "tags:", *[f"  - {base.yaml_string(tag)}" for tag in tags], 'author: "羽田野 剛士"', "draft: false", "featured: false", f"cta: {_cta(data)}", "---", "", str(data.get("summary", "")).strip(), "", str(data["body_markdown"]).strip()]
    faq = data.get("faq", [])[:5]
    if faq:
        lines.extend(["", "## よくある質問", ""])
        for item in faq:
...
def editorial_main() -> int:
    """Daily publisher main with output-level privacy validation.

    The shared knowledge generator retains its stricter seed gate for other use cases.
    This editorial entry point intentionally skips seed-level confidentiality rejection,
    lets the model sanitize/rebuild the seed, then validates only the public payload.
    """
    args = base.parse_args()
    timer = base.RunTimer()
    state = base.load_json(args.state_path, {"documents": {}})
    report: dict[str, Any] = {"selected": False, "runLog": [], "createdAt": base.now_iso()}
    if not args.folder_id:
        raise RuntimeError("GOOGLE_DRIVE_KNOWLEDGE_FOLDER_ID is required")
    ...
    if base.retry_count(record) > base.MAX_RETRIES:
    ...
    seed_flags = base.confidentiality_flags(doc.get("name", ""), source_text)
    if seed_flags:
        report["seedSanitizationRequired"] = True
        report["seedConfidentialityFlagCount"] = len(seed_flags)
    if args.dry_run:
    ...
    if residual_flags:
        mark_finished(state, doc, "skipped_confidential", {"reason": "sensitive material remained in generated public output", "score": score}, permanent=True)
        return base.finish(args, state, {**report, "score": score, "residualConfidentialityFlagCount": len(residual_flags)}, timer, False, "confidential_output")
    slug = base.normalize_slug(str(data.get("suggested_slug", "")), doc["id"])
    ...
    report.update({"selected": True, "slug": slug, "score": score, "eeat": data.get("eeat", {}), "researchReview": research_review, "outputs": [str(path.relative_to(ROOT)) for path in outputs], "seedConfidentialityFlagsSanitized": len(seed_flags), "aiSanitizationNotes": len([x for x in data.get("confidentiality_flags", []) if str(x).strip()])})
