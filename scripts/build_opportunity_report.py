...
def call_openai(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    prompt = (
        ...
        "LHub連携は現段階では評価対象から外してください。"
        "JSON配列のみを返し、各要素を "
        "{id, seo_score, inquiry_score, urgency_score, authority_score, total_score, "
        "target_segments, article_angle, recommended_cta, rationale} としてください。"
        "各scoreは0〜100。recommended_ctaはconsultationまたはself-pay。"
        "target_segmentsは日本語配列。total_scoreはSEO25%、問い合わせ35%、緊急性20%、一次情報信頼性20%を目安にしてください。\n\n"
...
def main() -> None:
    ...
    if not opportunities:
        lines.append("本日は記事化基準を満たす候補がありません。")
    else:
